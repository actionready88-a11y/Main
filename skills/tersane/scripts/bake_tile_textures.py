...
Çalıştırma:
    python3 bake_tile_textures.py <gemi.blend> <gemi_klasörü> [--tile 4.0] [--res 2048] [--skip MAT_A,MAT_B] [--tag v026]
...
_A = sys.argv[sys.argv.index("--") + 1:] if "--" in sys.argv else sys.argv[1:]


def _opt(name, default):
    return _A[_A.index(name) + 1] if name in _A else default


BLEND = Path(_A[0])
ROOT = Path(_A[1])
SHIP = ROOT.name
VER = _opt("--tag", BLEND.stem.split("_")[-1])
RES = int(_opt("--res", 2048))
TILE_M = float(_opt("--tile", 4.0))
OUT = ROOT / "Textures" / "tiles"
SKIP = set(filter(None, _opt("--skip", "").split(",")))
...
def main():
    ...
    for mat in sorted(used, key=lambda m: m.name):
        if mat.name in SKIP:
            manifest["not_baked"][mat.name] = "kullanıcı tarafından atlandı (--skip)"
            continue
